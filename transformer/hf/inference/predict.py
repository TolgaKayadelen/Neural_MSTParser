...
class BertInferencePipeline:
  def __init__(self, model_path, tokenizer_name="bert-base-multilingual-cased"):
    ...
    self.labeled_treebank = treebank_pb2.Treebank()
    self.eval_dir = os.path.join(_EVAL_DIR, f"bert-finetuned-{_CURRENT_TIME}-{os.path.basename(model_path)}")
  ...
  def predict_labels(self):
    ...
    for example in test_dataset:
      ...
      self._add_sentence_to_labeled_treebank(sentence, label_preds)

    gold_path = os.path.join(self.eval_dir, "gold_test_treebank.pbtxt")
    labeled_path = os.path.join(self.eval_dir, "labeled_test_treebank.pbtxt")
    self.write_treebank(test_treebank, path=gold_path)
    self.write_treebank(self.labeled_treebank, path=labeled_path)
    logging.info(f"All treebanks written to {self.eval_dir}")
    return gold_path, labeled_path
  ...
  def _add_sentence_to_labeled_treebank(self, sentence, labels):
    ...
    for label, token in zip(labels, sentence.token[1:]):
      ...
      parsed_sent_token.label = self.label_reader.itov(label)
      # print("parsed sentence now ", parsed_sentence)
      # print("gold sentence now ", sentence)
      # input()

    # print("parsed sentence ", parsed_sentence)
    # print("sentence ", sentence)
    # input()
    treebank_sentence = self.labeled_treebank.sentence.add()
