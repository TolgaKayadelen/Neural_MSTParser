...
class Evaluator:
  ...
  def uas_total(self) -> float:
    ...
    for gold_sent, test_sent in self.gold_and_test:
      # print("gold sentence ", gold_sent)
      assert (gold_sent.sent_id == test_sent.sent_id), "mismatching sent id"
      # input()
      # print(gold_sent.sent_id, test_sent.sent_id)
      # input()
      gold_heads = []
      pred_heads = []
      for token_gold, token_test in zip(gold_sent.token[1:], test_sent.token[1:]):
        if token_test.label != "punct":
          gold_heads.append(token_gold.selected_head.address)
          pred_heads.append(token_test.selected_head.address)
      # gold_heads = [tok.selected_head.address for tok in gold_sent.token if tok.label != "punct"]
      # print("gold heads ", gold_heads)
      total_heads += len(gold_heads)
      # print("total heads ", total_heads)
      # input()
      # pred_heads = [tok.selected_head.address for tok in test_sent.token if tok.label != "punct"]
  ...
  def las_total(self) -> float:
    ...
    for gold_sent, test_sent in self.gold_and_test:
      ...
      for token_gold, token_test in zip(gold_sent.token[1:], test_sent.token[1:]):
        if token_test.label != "punct":
          gold_heads.append((token_gold.selected_head.address, token_gold.label))
          pred_heads.append((token_test.selected_head.address, token_test.label))
      # gold_heads = [
      #   (tok.selected_head.address, tok.label) for tok in gold_sent.token if tok.label != "punct"
      # ]
      # pred_heads = [
      #   (tok.selected_head.address, tok.label) for tok in test_sent.token if tok.label != "punct"
      # ]
      # print("gold heads", gold_heads)
      total_heads += len(gold_heads)
      # print("pred_heads ", pred_heads)
      # input()
      assert len(gold_heads) == len(pred_heads), "Tokenization mismatch!!"
      las +=  sum(
        gh == ph for gh, ph in zip(gold_heads, pred_heads))
  ...
  def evaluate(self, *args):
    ...
    if self.write_results:
      if self.model_name:
        ...
        output_file_name = f"{current_time}_eval_results_no_punct"
